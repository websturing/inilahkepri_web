            ...
            <div class="ik-container">
                <div class="newsLeft">
                    <div v-for="(i,Iindex) in preiklanByAboveTitle" :key="Iindex">
                        <img :src="url.iklan+'/'+i.iklan_name" style="width:100% !important; border-radius:0px; margin-bottom:10px">
                    </div>
        ...
        computed: {
                ...
                if (this.filterCategory && this.filterCategory !== 'all') {
                    iklan = iklan.filter((p) => {
                        let foundCategory = p.iklan_position.findIndex((c) => {
                            return c.show_on === this.filterCategory && c.position === 'right_content'
                        })
            ...
            preiklanByAboveTitle() {
                ...
                if (this.filterCategory && this.filterCategory !== 'all') {
                    iklan = iklan.filter((p) => {
                        let foundCategory = p.iklan_position.findIndex((c) => {
                            return c.show_on === this.filterCategory && c.position === 'above_title'
                        })
                        return foundCategory !== -1
                    })
                }
